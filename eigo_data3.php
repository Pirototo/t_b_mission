...
if (isset($_POST["japanese"]) && isset($_POST["english"]) && isset($_POST["submit_btn"])&& $_POST["japanese"] != "" && $_POST["english"] != "" && isset($_POST["editnum"]) && $_POST["editnum"] != ""){
	if (($_POST["editnum"] > 0)){
		...
		$ja = $_POST["japanese"];
		$en = $_POST["english"]; 
		$sql = "update eigo3 set japanese='$ja' , english='$en' where id = $id";
...
elseif(isset($_POST["japanese"]) && isset($_POST["english"]) && isset($_POST["submit_btn"])&& $_POST["japanese"] != "" && $_POST["english"] != ""){
	 $sql = $pdo -> prepare("INSERT INTO eigo3 (japanese, english, date) VALUES (:japanese, :english, :date)");
...
<body>
    <form action = "eigo_data3.php" method = "POST">
...
<?php  
    ...
    //表の中身表示
    $sql = 'SELECT * FROM eigo3 ORDER BY id ASC';
	$results = $pdo -> query($sql);
	foreach ($results as $row){
		 //$rowの中にはテーブルのカラム名が入る
		 echo $row['id'].', ';
